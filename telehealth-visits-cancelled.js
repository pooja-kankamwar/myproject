...
module.exports.handler = async (event) => {
  ...
  try {
    ...
    utils.createLog(
      '',
      `getCancelledTelehealthVisits request validation Error`,
      error
    );
    return response;
  }
  ...
  try {
    utils.createLog('', `getCancelledTelehealthVisits request params`, params);
    const resultData = await telehealthService.getCancelledTelehealthVisits(
      params
    );
    utils.createLog('', `getCancelledTelehealthVisits Query Data`, resultData);
    ...
    utils.createLog('', `getCancelledTelehealthVisits SQL query Error`, error);
  }
  return response;
};
